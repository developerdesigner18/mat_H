...
class AuthController extends Controller
{
                    ...
                    if(!empty($super_admin_settings['free_trial_days']))
                    {
                        ...
                        if($trial_will_expire < time())
                        {
                            ...
                            return back()->withErrors([
                                'trial_expired' => __('Your trial has been expired. <a href="/renew_plan"> Renew Now</a>'),
                            ]);
                        }
                    }
                }
                if($workspace && $workspace->id != 1 && $workspace->trial == 0)
                {

                    if($workspace->next_renewal_date <= Carbon::now())
                    {

                            return back()->withErrors([
                                'trial_expired' => __('Your Subscription has been expired. <a href="/renew_plan"> Renew Now</a>'),
                            ]);
                    }
                }
            }
    ...
    public function signupPost(Request $request)
    {
        ...
        if(!empty($super_admin_settings['free_trial_days']))
        {
        // dd($super_admin_settings);
            $createdate = Carbon::now();
            $trial_days = $super_admin_settings['free_trial_days'];
            $date = $createdate->addDays($trial_days);
            // dd($date);
        }
        ...
        $workspace->next_renewal_date = $date->format('Y-m-d');
    ...
    public function Signup_subscribe(Request $request)
    {
        ...
        if ($plan) {
            ...
            return \view("settings.subscribes", [
                "selected_navigation" => "billing",
