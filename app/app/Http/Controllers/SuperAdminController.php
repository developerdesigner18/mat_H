...
class SuperAdminController extends SuperAdminBaseController
{
    ...
    {
        $users = User::where('super_admin','!=',2)->get();
    ...
    public function saasCoupons()
    {
        return \view("super-admin.Coupons", [
            "selected_navigation" => "saas-Coupons",
            // "plans" => $plans,
        ]);
    }

    public function createSaasCoupon(Request $request)
    {
        $number = $request->couponnumber;
        for($i = 0; $i<$number; $i++)
        {
            $randcode = Str::random(10);
            $couponcode  = DB::table('coupon')->insert(
                [
                    'Coupon_name' => $request->couponnae,
                    'coupon_text'=>$randcode,
                    'coupon_discount'=>100,
        ...
        $i++;
        return response()->json(['success'=>'1','response'=>'Coupon created successfully!']);
    }

    public function ShowCoupon()
    {
        // $coupon = Coupon::select('Coupon.*','workspaces.name')->join('workspaces', 'workspaces.id', '=', 'Coupon.UserId')->toSql();
        $coupon = DB::table('coupon')->where('active',1)->orWhere('active',0)->get();
        foreach($coupon as $val)
        {
            if($val->UserId != NULL || $val->UserId != '') 
            {
                $username = Workspace::where('id',$val->UserId)->first();
                $val->UserId = $username->name;
            }
        }
        return ($coupon);
    }
    ...
    {
        $getcoupons =  DB::table('coupon')->get();
        foreach($getcoupons as $val)
        {
            if($val->active == 0)
            {
                $deactive = DB::table('coupon')->where('active',0)->update(['active'=>2]);
            }
            else
            {
                if($val->Validity < Carbon::now())
                {
                    $deactive = DB::table('coupon')->where('active',1)->where('Validity','<',Carbon::now())->delete();
                }
            }
        }
        return response()->json(['success'=>1,'response'=>'Expired coupons Deleted succesfully!']);
        
        // $getcoupons =  DB::table('coupon')->where('active',1)->Where('Validity','<',Carbon::now())->get();
        // $couponCount = $getcoupons->count();
        //     if($couponCount>0)
        //     {
        //         $deletecoupon = DB::table('coupon')->where('active',1)->Where('Validity','<',Carbon::now())->delete();
        //         if($deletecoupon)
        //         {
        //             return response()->json(['success'=>'1','response'=>'Coupons Deleted !']);
        //         }
        //         else
        //         {
        ...
        //     else{
        //         return response()->json(['success'=>'0','response'=>'Nothing to delete !']);
        //     }   

        //     $getdeactivecoupon =  DB::table('coupon')->where('active',0)->get();
        //      $deactivecouponCount = $getcoupons->count();
        //     if($deactivecouponCount>0)
        //     {
        //         $deactive = DB::table('coupon')->where('active',0)->update(array(
        //             'active'=>0));
        //         if($deactive)
        //         {
        //             return response()->json(['success'=>'1','response'=>'Coupons Deleted !']);
    ...
    public function configurePaymentGatewayPost(Request $request)
    {
        ...
        if($api_name === 'paypal')
        {
            ...
            $payment_gateway->api_name = $api_name;
            $payment_gateway->public_key = $request->username;
            $payment_gateway->private_key = $request->password;
        }
    ...
    public function super_admin_userpost(Request $request)
    {
        $request->validate([
        ...
        $existcoupon = DB::table('coupon')->where('coupon_text',$request->barcode)->first();
        
        if($existcoupon && $existcoupon->active !=0)
        {
         
            $workspace = new Workspace();
            $workspace->name = $request->first_name . "'s workspace";
            $workspace->subscription_start_date = Carbon::now()->format('Y-m-d');
            $workspace->subscribed = 1;
            ...
            DB::table('coupon')->where('coupon_text',$request->barcode)->update(array(
                'active'=>0,
                'UserId'=>$workspace->id
            ));
