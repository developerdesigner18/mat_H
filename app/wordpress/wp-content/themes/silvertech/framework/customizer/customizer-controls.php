...
class Silvertech_Customizer_Hr_Control extends WP_Customize_Control {
	public $type = 'hr';
...
class Silvertech_Customizer_Heading_Control extends WP_Customize_Control {
	public $type = 'silvertech-heading';
	public function render_content() {
		echo '<span class="silvertech-customizer-heading">'. esc_html( $this->label ) .'</span>';
	}
}

// Textarea control for the customizer
class Silvertech_Customizer_Textarea_Control extends WP_Customize_Control {
	public $type = 'silvertech_textarea';
	public $rows = '3';
	public function render_content() { ?>
		<label>
...
class Silvertech_Customize_Multicheck_Control extends WP_Customize_Control {
	public $description = '';
	public $subtitle = '';
	...
	public function render_content() {
		...
		if ( self::$firstLoad ) {
			...
			jQuery(document).ready( function( $ ) {
				"use strict";
				$( 'input.silvertech-multicheck' ).change( function(event) {
		...
		<label class='silvertech-multicheck-container'>
			...
			foreach ( $this->choices as $value => $label ) {
				printf( '<label for="%s"><input class="silvertech-multicheck" id="%s" type="checkbox" value="%s" %s/> %s</label><br>',
					$this->id . $value,
					$this->id . $value,
					esc_attr( $value ),
...
// Sorter Control
class Silvertech_Customize_Control_Sorter extends WP_Customize_Control {
	...
	public function render_content() { ?>
		<div class="silvertech-sortable">
			...
			echo '<ul id="'. $this->id .'_sortable">'; ?>
				...
				foreach ( $values as $val ) :
					...
					if ( $label ) : ?>
						<li data-value="<?php echo esc_attr( $val ); ?>" class="silvertech-sortable-li">
							<?php echo esc_html( $label ); ?>
							<span class="silvertech-hide-sortee fa fa-toggle-on"></span>
						</li>
					<?php
					// End if label check
					endif;
				...
				foreach ( $choices as $val => $label ) { ?>
					<li data-value="<?php echo esc_attr( $val ); ?>" class="silvertech-sortable-li silvertech-hide">
						<?php echo esc_html( $label ); ?>
						<span class="silvertech-hide-sortee fa fa-toggle-on fa-rotate-180"></span>
					</li>
				<?php } ?>
			</ul>
		</div><!-- .silvertech-sortable -->
		...
		jQuery(document).ready( function($) {
			...
			sortableUl.on( "sortstop", function( event, ui ) {
				silvertechUpdateSortableVal();
			} );

			// Toggle classes
			sortableUl.find( 'li' ).each( function() {
				$( this ).find( '.silvertech-hide-sortee' ).on('click', function() {
					$( this ).toggleClass( 'fa-rotate-180' ).parents( 'li:eq(0)' ).toggleClass( 'silvertech-hide' );
				} );
			})
			// Update Sortable when hidding/showing items
			$( '#<?php echo esc_html( $this->id ); ?>_sortable span.silvertech-hide-sortee' ).on( 'click', function() {
				silvertechUpdateSortableVal();
			} );
			// Used to update the sortable input value
			function silvertechUpdateSortableVal() {
				var values = [];
				sortableUl.find( 'li' ).each( function() {
					if ( ! $( this ).hasClass( 'silvertech-hide' ) ) {
...
class Silvertech_Fonts_Dropdown_Custom_Control extends WP_Customize_Control {
	public function render_content() {
	$this_val = $this->value(); ?>
	<label>
		<span class="customize-control-title"><?php echo esc_html( $this->label ); ?></span>
			<select <?php $this->link(); ?> style="width:100%;">
			<option value="" <?php if ( ! $this_val ) echo 'selected="selected"'; ?>><?php esc_html_e( 'Default', 'silvertech' ); ?></option>
			<?php
			// Get Standard font options
			$std_fonts = '';
			if ( $std_fonts = silvertech_standard_fonts() ) { ?>
				<optgroup label="<?php esc_attr_e( 'Standard Fonts', 'silvertech' ); ?>">
					<?php
			...
			// Google font options
			$google_fonts = '';
			if ( $google_fonts = silvertech_google_fonts_array() ) { ?>
				<optgroup label="<?php esc_attr_e( 'Google Fonts', 'silvertech' ); ?>">
					<?php
					// Loop through font options and add to select
