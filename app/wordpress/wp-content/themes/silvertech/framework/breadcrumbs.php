...
function silvertech_breadcrumbs() {

    global $post;

    $home_text     = esc_html__( 'Home','silvertech' );
    $category_text = esc_html__( 'Archive by Category "%s"','silvertech' );
    $tax_text      = esc_html__( 'Archive by "%s"','silvertech' );
    $tag_text      = esc_html__( 'Posts Tagged "%s"','silvertech' );
    $author_text   = esc_html__( 'Articles Posted by %s','silvertech' );
    $error_text    = esc_html__( 'Error 404','silvertech');
    $search_text   = esc_html__( 'Search Results for "%s" Query','silvertech' );

    $blog_text     = esc_html__( 'Blog','silvertech' );
    $project_text  = esc_html__( 'Projects','silvertech' );
    $product_text  = esc_html__( 'Shop','silvertech' );
    $service_text  = esc_html__( 'Service','silvertech' );
            ...
            foreach ( $cpt_list as $cpt ) {
                ...
                if ( $cpt->name == 'project' ) {
                    if ( is_singular( 'project' ) ) {
                        ...
                        if ( $page_id = silvertech_parse_obj_id( silvertech_get_mod( 'portfolio_page' ), 'page' ) ) {
                    ...
                    if ( is_singular( 'service' ) ) {
                        ...
                        if ( $page_id = silvertech_parse_obj_id( silvertech_get_mod( 'service_page' ), 'page' ) ) {
                            $page_permalink = get_permalink( $page_id );
                            $page_name      = get_the_title( $page_id );
                            if ( $page_permalink && $page_name ) {
                                echo '<a href="'. esc_url( $page_permalink ) .'" title="'. esc_attr( $page_name ) .'">'. esc_html( $page_name ) .'</a>';
